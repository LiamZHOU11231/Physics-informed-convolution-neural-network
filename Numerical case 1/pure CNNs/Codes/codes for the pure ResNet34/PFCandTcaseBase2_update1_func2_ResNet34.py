...
def channels_stack(image_npy):
    image_channels = []
    for i in range(5):
        image = np.reshape(image_npy[:, i], (32, 32))
        image_channels.append(image)

    image_channels  = np.stack(image_channels, axis=2)
    FinalData       = np.transpose(image_channels, (2, 0, 1))
    # 转为通道数在前面，即格式为：input_channels, input_height, input_width。
...
# ---------------------------------------------------------------------------------------------------
class DB_PFCandTcaseBase2_update1_ResNet34(data.Dataset):
# ---------------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------
        data_dir = "D:\RFEMNN_InPyCharm\project1\ForPFCandT_NEW\PFCandT_caseBase2_update1\DB_Input_70dB\\"
# ---------------------------------------------------------------------------------------------------
    ...
    def __getitem__(self, index):
        ...
        # 转成 ndarray:32 * 32 * 5
        # 相较于不带"_ResNet"后缀的func2，多了这一步。
        # ⭐️ ----------------------------------------- ⭐️

        img = channels_stack(img)
