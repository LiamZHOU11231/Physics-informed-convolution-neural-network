...
# Caution 1/6  导入dataset数据集 ================================================================================
# -------------------------------------------------------------------------------------------------------------
from PFCandTcase1_update17_e2v3_func2 import DB_PFCandTcase1_update17_e2v3
from PFCandTcase1_update17_e2v3_func3_Kernel72 import MyConvNet
# -------------------------------------------------------------------------------------------------------------

# Caution 2/6  确认损失函数 =======================================================================================
# -------------------------------------------------------------------------------------------------------------
from PFCandTcase1_update17_e2v3_func_LossErrReal_e3 import LossErrReal_USE
# -------------------------------------------------------------------------------------------------------------

if __name__ == '__main__':

# Caution 3/6  指定测试集输出的保存路径 ==========================================================================
# -------------------------------------------------------------------------------------------------------------

    loc_test_out = 'D:\RFEMNN_InPyCharm\project1\ForPFCandT_NEW\PFCandT_case1_update17_e2v3\PFCandT_case1_update17_e2v3_result_70dB' \
                    + '\OutputTrain_fromLossErrRealE3_ShuffleFalse_NETother9.xlsx'
    loc_test_outGT = 'D:\RFEMNN_InPyCharm\project1\ForPFCandT_NEW\PFCandT_case1_update17_e2v3\PFCandT_case1_update17_e2v3_result_70dB' \
                    + '\OutputTrainGT_fromLossErrRealE3_ShuffleFalse.xlsx'

# -------------------------------------------------------------------------------------------------------------

# Caution 4/6  导入训练好的参数 ================================================================================
# -------------------------------------------------------------------------------------------------------------
    model_weight_path = 'D:\RFEMNN_InPyCharm\project1\ForPFCandT_NEW\PFCandT_case1_update17_e2v3' \
                        '\PFCandT_case1_update17_e2v3_weight_other_70dB' \
                        + '\MyConvNet_other9.pth'
# -------------------------------------------------------------------------------------------------------------

# Caution 5/6  测试集涉及的元素个数、网络参数指定 ==================================================================
# -------------------------------------------------------------------------------------------------------------
# 样本参数指定
    batchS_test     = 1
    input_height    = 1024
...
# Caution 6/6  加载数据 ========================================================================================
# ⭐️ ------------------------------------------------------ ⭐️
    test_dataset = DB_PFCandTcase1_update17_e2v3(mode='train')
...
# ⭐️ ------------------------------------------------------ ⭐️
    ...
    # 网络进入测试模式
    net_USE.eval()
    running_loss1_MAE_test      = 0.0
    running_loss1_ErrReal_test  = 0.0
    idx_test = 0
    with torch.no_grad():
        ...
        # 这样就可以保证在验证阶段，不改变网络权重。
            ...
            for sample in test_bar:
                ...
                # 计算预测结果的损失
                loss1_MAE_test      = loss_function(outputs_test, gt_test.to(device))
                loss1_ErrReal_test  = LossErrReal_USE(batchS_test, outputs_test, gt_test.to(device))

                # 计算MAE、Err
                running_loss1_MAE_test += loss1_MAE_test.item()
                running_loss1_ErrReal_test += loss1_ErrReal_test.item()

    MAE_1_test          = running_loss1_MAE_test / test_iters
    ErrReal_1_test      = running_loss1_ErrReal_test / test_iters
